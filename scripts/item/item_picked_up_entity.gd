...
func drop(position : Vector2):
	var entity = Game.spawn_entity(metadata.entity_id, position, metadata.entity_metadata.duplicate())
...
func _on_use_started(user):
	var entity = Game.spawn_entity(metadata.entity_id, user.global_position, metadata.entity_metadata.duplicate())
	print(entity)
	if entity:
